...
public struct OrganismModel {
    public let shape: MLPNodeShape // holds layer sizes
    internal var weights: ModelWeights

    public init(
        shape: MLPNodeShape = MLPNodeShape(inputNodesCount: 8, hiddenNodesCount: 24, outputNodesCount: 4),
        inputToHiddenWeights: [Float]? = nil,
        inputToHiddenBias: [Float]? = nil,
        hiddenToOutputWeights: [Float]? = nil,
        hiddenToOutputBias: [Float]? = nil
    ) {
        ...
        self.weights = ModelWeights(
            inputToHiddenWeights: inputToHiddenWeights ?? (0..<inHidCount).map { _ in .random(in: -1.0...1.0) },
            inputToHiddenBias: inputToHiddenBias ?? (0..<shape.hiddenNodesCount).map { _ in .random(in: -1.0...1.0) },
            hiddenToOutputWeights: hiddenToOutputWeights ?? (0..<hidOutCount).map { _ in .random(in: -1.0...1.0) },
            hiddenToOutputBias: hiddenToOutputBias ?? (0..<shape.outputNodesCount).map { _ in .random(in: -0.1...0.1) }
        )
    }
